...
两条案件都共享同一条论文学习链：先确认论文主张，再用具体证物交叉询问，保存带解释的学习笔记，比较竞争性假设，最后形成可复核的结论。论文材料位于 [src/papers.ts](./src/papers.ts)，因此后续可以继续添加其它论文而不改动庭审组件。
...
- 保存学习笔记需要玩家选择一条论文主张、至少一张关联证物，并写下自己的解释与标题。
...
已按首次用户路径分别试玩两篇论文：选择案件 → 阅读摘要 → 故意选错一次 → 根据右侧论文卷宗纠正 → 保存学习笔记 → 调查假设 → 合议结论 → 5/5 结案。过程中把“下一步该看什么”改成自动聚焦证物卡，并在选案页增加论文类型、年份、作者、摘要和 arXiv 原文链接；移动端也验证了选案与 4 张论文证物卡没有横向溢出。
