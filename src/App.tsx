...
export function App() {
  ...
  const allEvidence = useMemo(() => [paperEvidence(activePaper), ...paperEvidenceItems(activePaper), ...CORE_EVIDENCE], [activePaper])
  ...
  const present = (item: Evidence) => {
    ...
    if (item.concept !== 'commit') {
      ...
      raiseObjection('这件证物是真的，但不能解释“历史被覆盖”的矛盾。', item.concept)
  ...
  return (
    <div className="game game--trial">
      ...
      {objection ? <div className="objection-flash" role="alert" aria-live="assertive"><strong>OBJECTION!</strong><span>{objection}</span></div> : null}
      <main className="court-layout">
        ...
        <div className="court-grid">
          ...
          <aside className="board">
            ...
            <div className="lesson-card"><div className="lesson-card-head"><span>{activeEvidence.concept === 'paper' ? 'PAPER EVIDENCE' : 'CONCEPT NOTE'}</span><span>{activeEvidence.label}</span></div><h3>{activeEvidence.title}</h3>{activeEvidence.value ? <strong className="lesson-metric">{activeEvidence.value}</strong> : null}<p>{activeEvidence.description}</p><code>{activeEvidence.command}</code></div>
...
function Verdict({ score, paper, onRestart }: { score: number; paper: PaperCase; onRestart: () => void }) {
  const receiptItems = [paperEvidence(paper), ...paperEvidenceItems(paper).slice(0, 2), ...CORE_EVIDENCE]
